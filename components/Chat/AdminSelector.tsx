    ...
    if (loading) {
        return (
            <div className={`flex flex-col items-center justify-center text-slate-400 ${compact ? 'p-4' : 'p-8'}`}>
                <Loader2 className={`${compact ? 'w-6 h-6' : 'w-8 h-8'} animate-spin mb-2 text-[#E06B8B]`} strokeWidth={1.2} />
            ...
            <div className={`grid ${compact ? 'grid-cols-1 gap-2' : 'grid-cols-1 sm:grid-cols-2 gap-4'} w-full max-w-2xl`}>
                {admins.map((admin) => (
                    <button
                        ...
                        className={`flex items-center gap-3 rounded-2xl bg-slate-800/50 hover:bg-slate-800 border border-white/5 hover:border-[#E06B8B]/50 transition-all group text-left ${compact ? 'p-3' : 'p-4'}`}
                    >
                        <div className={`relative rounded-xl overflow-hidden bg-slate-700 shrink-0 border-2 border-transparent group-hover:border-[#E06B8B] transition-colors ${compact ? 'w-10 h-10' : 'w-12 h-12'}`}>
                            {admin.image ? (
                        ...
                        <div className="flex-1 min-w-0">
                            <h3 className={`font-semibold text-white group-hover:text-[#E06B8B] transition-colors truncate ${compact ? 'text-sm' : 'text-base'}`}>
                        ...
                        <div className={`rounded-xl bg-white/5 flex items-center justify-center group-hover:bg-[#E06B8B] group-hover:text-white transition-all ${compact ? 'w-8 h-8' : 'w-10 h-10'}`}>
